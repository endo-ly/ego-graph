...
| **認証方式** | なし（上流で取得済み） |
| **必要な情報** | URL, title, started_at_utc, source_device, browser, profile |
...
| `video_url` | STRING | 正規URL | URL正規化 |
| `video_title` | STRING | 動画タイトル | `title` または YouTube Data API |
| `channel_id` | STRING | チャンネルID | YouTube Data API |
| `channel_name` | STRING | チャンネル名 | YouTube Data API |
| `content_type` | STRING | `video` / `short` | URL正規化 |
| `updated_at_utc` | TIMESTAMP | 更新時刻 | システム生成 |
...
s3://ego-graph/
  ...
  │       └── {uuid}.parquet
  └── state/
      └── youtube_ingest_state.json
```

### 5.2 保存パス例

- **Watch Events**: `s3://ego-graph/events/youtube/watch_events/year=2026/month=04/abc123.parquet`
- **Videos Master**: `s3://ego-graph/master/youtube/videos/def456.parquet`
- **Channels Master**: `s3://ego-graph/master/youtube/channels/ghi789.parquet`
- **Raw**: `s3://ego-graph/raw/browser_history/2026-04-21T120000.json` を原本として参照
- **State**: `s3://ego-graph/state/youtube_ingest_state.json`
